...
from model import upload_bank_file, get_extracted_values,upload_pdf_to_blob, download_file
...
@app.get("/view_adhar")
def view_adhar():
    """Route to download and view the Aadhaar PDF."""
    try:
        file_path = download_file("aadhar_file", "pdf")
        return FileResponse(file_path, media_type="application/pdf", filename="adhar.pdf")
...
@app.get("/view_pan")
def view_pan():
    """Route to download and view the PAN PDF."""
    try:
        file_path = download_file("pan_file", "pdf")  # Update blob name if necessary
        return FileResponse(file_path, media_type="application/pdf", filename="pan.pdf")
    except Exception as e:
...
def view_bank_statement():
    ...
    try:
        file_path = download_file("Bank_Statement", "csv")  # Update blob name if necessary
