...
# PREDICTION_URL = "https://bankclass-prediction.cognitiveservices.azure.com/customvision/v3.0/Prediction/0fcd9dd4-d8eb-4292-bb15-b3c4819fcc18/classify/iterations/Iteration8/image"
...
#         image_stream = process_pdf(file_path)  # Call your process_pdf function
...
#             analyze_pdf(file_path,None,None,None)  # Analyze the PDF for tables
...
#                 account_name = fields_df.loc[fields_df["Field Name"].str.contains("account name", case=False, na=False), "Value"]
#                 account_no = fields_df.loc[fields_df["Field Name"].str.contains("account", case=False, na=False) & fields_df["Field Name"].str.contains("no", case=False, na=False), "Value"]
#                 address = fields_df.loc[fields_df["Field Name"].str.contains("address", case=False, na=False), "Value"]
#                 branch = fields_df.loc[fields_df["Field Name"].str.contains("branch", case=False, na=False), "Value"]

#                 account_name_str = account_name.iloc[0] if not account_name.empty else "NotFound"
...
#                 concatenated_info = f"{account_name_str}{account_no_str}{branch_str}"
...
def download_pdf(blob_name):
    """Download a PDF file from Azure Blob Storage for viewing."""
    blob_name = f"{folder_name}/{blob_name}.pdf"
    try:
        blob_client = blob_service_client.get_blob_client(container=CONTAINER_NAME, blob=blob_name)
        ...
        local_file_path = f"./downloads/{blob_name}.pdf"
        os.makedirs(os.path.dirname(local_file_path), exist_ok=True)
        with open(local_file_path, "wb") as local_file:
            local_file.write(download_stream.readall())
        
        return local_file_path  # Return the path for viewing
...
def download_csv(blob_name):
    ...
    try:
        ...
        # Save the file locally to serve it via FastAPI
        local_file_path = f"./downloads/{blob_name}.csv"
        os.makedirs(os.path.dirname(local_file_path), exist_ok=True)
        with open(local_file_path, "wb") as local_file:
            local_file.write(download_stream.readall())
        
        return local_file_path  # Return the path for further processing or download

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error downloading CSV file: {str(e)}")


def download_file(blob_name: str, file_extension: str):
